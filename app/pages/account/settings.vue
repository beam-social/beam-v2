...
onMounted(async () => {
	...
	axios
		.get(`${$apiUrl}/me/entitlements/badges`, {
			withCredentials: true,
		})
		.then((response) => {
...
function saveProfile() {
	axios
		.put(`${$apiUrl}/me/profile`, profile.value, {
			withCredentials: true,
		})
		.then(() => {
...
function saveSettings() {
	axios
		.put(`${$apiUrl}/me/settings`, settings.value, {
			withCredentials: true,
		})
		.then(() => {
...
function uploadAvatar() {
	...
	formData.append(
		"avatar",
		(document.getElementById("avatarInput") as HTMLInputElement).files![0]!,
	);

	axios
		.post(`${$apiUrl}/me/avatar`, formData, {
			withCredentials: true,
			headers: {
...
<template>
	<main class="flex flex-col p-4 gap-4 xs:p-8">
		<h1 v-if="section == 'home'" class="text-4xl text-center font-bold">
			Paramètres
		</h1>
		<div v-else class="flex items-center gap-4">
			<button
				@click="section = 'home'"
				class="cursor-pointer flex items-center gap-1 bg-surface text-on-surface font-medium border border-surface-border rounded-full w-fit px-4 py-2"
			>
				<ChevronRightIcon
					class="w-4 h-4 rotate-180 stroke-2 stroke-muted text-muted"
				/>
				<span>Retour</span>
			</button>
		</div>
		<section v-if="section == 'home'" class="space-y-8">
			<div class="space-y-1">
				<div class="grid grid-cols-1 gap-2 xl:grid-cols-2">
					<button
						class="cursor-pointer flex gap-1 items-center bg-surface text-on-surface font-medium border border-surface-border rounded-3xl w-full px-6 py-6"
					...
					>
						...
						<span class="grow text-lg text-left line-clamp-1"
							>Gérer mon grade</span
						>
						<ChevronRightIcon
							class="w-4 h-4 stroke-2 stroke-[#d013ff] text-[#d013ff]"
						/>
					</button>
				</div>
			</div>
			<div class="space-y-1">
				<h2 class="text-muted font-medium uppercase px-6">
					Compte & Sécurité
				</h2>
				<div
			...
			<div class="grid gap-8 max-sm:grid-cols-1 sm:grid-cols-2">
				<div class="shrink-0 w-48 sm:w-56">
					...
					<label for="avatarInput" class="cursor-pointer">
						<img
							:src="me?.avatar || ''"
							class="rounded-full w-48 h-48 sm:w-56 sm:h-56 mb-4"
						/>
					</label>
					<p class="text-muted text-sm">
						Cliquez sur la photo de profil pour la modifier.
					</p>
				</div>
				<div class="space-y-6 w-full">
					<div class="space-y-4">
						<label class="block font-medium"
							...
							<input
								...
								class="bg-surface text-sm border border-surface-border rounded-lg px-4 py-2 w-full sm:w-48 lg:w-64"
							/>
							<p class="text-muted text-sm mt-2">
								https://beam.ejnalo.me/@{{ profile.name }}
							</p>
						</label>
						<label class="block font-medium"
							>Nom d'affichage<br />
						...
						<label class="block font-medium"
							>Badge<br />
							<select
								v-model="profile.badge"
								class="cursor-pointer bg-surface text-sm border border-surface-border rounded-lg px-4 py-2 mr-2"
							>
					...
					<button
						class="cursor-pointer bg-button text-white text-sm font-medium rounded-full w-fit px-5 py-3 duration-300 hover:bg-button-hovered"
						@click="() => saveProfile()"
					>
						Enregistrer
					</button>
			...
			<div class="grid grid-cols-2 gap-4">
				<label class="block font-medium"
					...
					<input
						...
						class="bg-surface text-sm border border-surface-border rounded-lg px-4 py-2 w-full"
					/>
				</label>
				<label class="block font-medium opacity-50"
					>Statut <br />
					<input
						type="text"
				...
				<label class="block font-medium col-span-2"
					>Description<br />
					<textarea
		...
		<section v-if="section == 'plus'" class="px-4 space-y-8">
			...
			<div class="text-center">
				<ProfileBadge
					v-if="me?.profile.badge"
					:badge="me.profile.badge"
					class="w-20 h-20 mx-auto mb-2"
				/>
				<div
					v-else
					class="bg-surface w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-2"
				>
					<UserIcon class="w-10 h-10 text-on-surface" />
				</div>
				<h2 class="text-2xl font-bold">
					{{ me?.profile.badge?.title || "Aucun badge" }}
				</h2>
				<p class="text-muted text-center">
					Grade {{ grades[me?.profile.level || 3] }}
				</p>
			</div>
			...
			<div v-if="me?.profile.level === 1">
				<p>
					Vous ne pouvez plus poster sur Beam mais vous pouvez encore
					lire les posts.
				</p>
			</div>
			...
			<div
				v-if="me?.profile.level && me.profile.level >= 5"
				class="space-y-2"
			>
				<h3 class="text-xl font-bold">Avantages</h3>
				<ul class="list-disc list-inside">
			...
			<div class="space-y-4">
				...
				<div v-if="badges.length == 0" class="text-center text-muted">
			...
			<div class="space-y-4">
				...
				<div v-else class="flex flex-col gap-2">
					<div
						...
						:key="entitlement.id"
						class="bg-surface text-on-surface border border-surface-border rounded-2xl px-5 py-4 space-y-1"
						:class="
							entitlement.revoked ||
							(entitlement.expires_at &&
								entitlement.expires_at <= new Date())
								? 'opacity-50'
								: ''
						"
					>
						...
						<div>
							<p v-if="entitlement.expires_at" class="text-sm">
								Valable du
								{{
									new Date(
							...
							<p v-if="entitlement.reason" class="text-sm">
		...
		<section v-if="section == 'account'" class="px-4 space-y-8">
			...
			<div class="space-y-6">
				...
				<div class="space-y-2">
					<label class="block font-medium text-muted"
						>Nom d'utilisateur<br />
						<input
							type="text"
							v-model="credentials.username"
							disabled
							class="cursor-not-allowed bg-surface text-sm border border-surface-border rounded-lg px-4 py-2 w-auto"
						/>
					</label>
					<label class="block font-medium text-muted"
						>Adresse email<br />
						<input
							...
							disabled
							class="bg-surface text-sm border border-surface-border rounded-lg px-4 py-2 w-auto"
						/>
						<button
							@click="
								router.push('/auth/sudo?action=reset-email')
							"
							class="ml-2 bg-button text-white text-sm px-4 py-2 rounded-full hover:bg-button-hovered transition"
						>
							Modifier
						</button>
					</label>
				...
				<button
					@click="router.push('/auth/sudo?action=reset-password')"
					class="bg-button text-white text-sm px-4 py-2 rounded-full hover:bg-button-hovered transition"
				>
			...
			<div class="space-y-2">
				...
				<button
					@click="router.push('/auth/sudo?action=delete-account')"
					class="block bg-danger text-white px-4 py-2 rounded-full hover:bg-danger-hovered transition"
				>
		...
		<section v-if="section == 'privacy'" class="px-4 space-y-8">
			...
			<div class="space-y-4">
				...
				<label class="block font-medium"
					>Profil
					<select
						v-model="settings.privacy.profile"
						class="bg-surface text-sm border border-surface-border rounded-lg ml-4 px-2 py-2 w-64"
					>
				...
				<label class="block font-medium"
					>Statut
					<select
						v-model="settings.privacy.status"
						class="bg-surface text-sm border border-surface-border rounded-lg ml-4 px-2 py-2 w-64"
					>
				...
				<label class="block font-medium"
					>Anniversaire
					<select
						v-model="settings.privacy.birthday"
						class="bg-surface text-sm border border-surface-border rounded-lg ml-4 px-2 py-2 w-64"
					>
						<option value="everyone">Tout le monde</option>
						<option value="followers">Mes abonnés</option>
			...
			<div class="select-none cursor-not-allowed space-y-4 opacity-50">
				...
				<label class="cursor-not-allowed block font-medium"
					>Qui peut m'envoyer des messages directs ?<br />
					<select
						v-model="settings.privacy.dms"
						disabled
						class="bg-surface text-sm border border-surface-border rounded-lg px-2 py-2 w-64"
					>
						<option value="everyone">Tout le monde</option>
						<option value="followers">Mes abonnés</option>
						<option value="friends">Mes amis</option>
			...
			<div class="space-y-4">
				...
				>
					<ThemePreviewCard
						...
						@click="
							() => {
								settings.appearance.global_theme = theme;
								setTheme(theme);
							}
						"
					/>
