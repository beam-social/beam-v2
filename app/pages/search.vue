...
onMounted(async () => {
	...
	axios
		.get(`${$apiUrl}/users/fetch?q=${encodeURIComponent(query)}`, {
	...
	axios
		.get(`${$apiUrl}/posts/fetch?q=${encodeURIComponent(query)}`, {
			withCredentials: true,
		})
		.then((response) => {
...
<template>
	<main class="flex flex-col p-4 gap-4 xs:p-8">
		...
		<nav
			class="sticky z-500 top-4 flex gap-1 bg-surface backdrop-blur-xl text-on-surface border border-surface-border rounded-full w-fit p-2"
		>
			<button
