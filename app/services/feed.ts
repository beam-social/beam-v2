...
export const useFeed = () => {
	...
	const getFeed = async () => {
		...
		if (me.value) {
			_posts = (
				await axios.get(`${$apiUrl}/me/feed`, {
					withCredentials: true,
				})
			).data.posts;
		} else {
			_posts = (await axios.get(`${$apiUrl}/top/posts?sort_by=date`))
				.data;
		}
	...
	const getLeaderboard = async (tag?: string) => {
		...
		_posts = (
			await axios.get(`${$apiUrl}/top/posts?sort_by=stats`, {
				params: {
					tag,
					after: new Date(
		...
		let _users = (await axios.get(`${$apiUrl}/top/users`)).data;
